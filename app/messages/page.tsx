...
export default function MessagesPage() {
  ...
  // Load friends once authed
  useEffect(() => {
    if (!ready || !userId) return;
    (async () => {
      const { data: pairs } = await supabase.from("friendships").select("a,b").or(`a.eq.${userId},b.eq.${userId}`);
  ...
  if (!userId) return <div className="max-w-5xl mx-auto p-4 sm:p-6"><a className="btn btn-brand" href="/login">Sign in to use Messages</a></div>;
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-[220px_1fr]">
        <div className="card p-3">
          ...
          <div className="space-y-1">
            {friends.map(f => (
              <button key={f.id} className={`w-full text-left px-2 py-1 rounded ${to === f.id ? "bg-zinc-100" : "hover:bg-zinc-50"}`} onClick={() => setTo(f.id)}>
                {f.full_name || "Member"}
              </button>
            ))}
          ...
          {active ? (
            <>
              ...
              <div className="mt-3 flex gap-2">
                <input className="input flex-1" placeholder="Type a message…" value={body} onChange={(e) => setBody(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") send(); }} />
