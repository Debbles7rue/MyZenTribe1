// components/PostsFeed.tsx - WITH DEBUG LOGGING
"use client";
...
export default function PostsFeed({ 
  ...
  async function loadUserPosts() {
    ...
    try {
      // First, let's check both tables to see where the data is
      console.log('🔍 DEBUG - Checking feed_posts table...');
      ...
      console.log('🔍 DEBUG - feed_posts query result:', { 
        data: feedPostsData, 
        error: feedPostsError,
        count: feedPostsData?.length || 0
      });

      // Also check the posts table to see if data is there instead
      console.log('🔍 DEBUG - Checking posts table...');
      ...
      console.log('🔍 DEBUG - posts table query result:', { 
        data: originalPostsData, 
        error: originalPostsError,
        count: originalPostsData?.length || 0
      });

      // Use whichever table has data
      let postsData = feedPostsData;
      let postsError = feedPostsError;

      if ((!feedPostsData || feedPostsData.length === 0) && originalPostsData && originalPostsData.length > 0) {
        console.log('✅ DEBUG - Using posts table data instead');
        postsData = originalPostsData;
        postsError = originalPostsError;
      }
      ...
      if (!postsData || postsData.length === 0) {
        console.log('📭 DEBUG - No posts found in either table');
        setPosts([]);
        return;
      }

      console.log('✅ DEBUG - Found posts:', postsData.length);
      ...
      console.log('👤 DEBUG - Author profile:', profile);
      ...
      console.log('🎨 DEBUG - Formatted posts:', formattedPosts);
    ...
    return (
      <div className="posts-empty">
        ...
        <p style={{ fontSize: '12px', color: '#666', marginTop: '16px' }}>
          DEBUG: Searched for posts by user {userId} in both feed_posts and posts tables
        </p>
        <style jsx>{`
          .posts-empty {
            display: flex;
  ...
  return (
    <div className="posts-feed">
      <div style={{ fontSize: '12px', color: '#666', marginBottom: '16px', textAlign: 'center' }}>
        DEBUG: Showing {posts.length} posts for user {userId}
      </div>
