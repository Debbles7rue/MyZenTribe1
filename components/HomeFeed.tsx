...
export default function HomeFeed() {
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 sm:p-6">
      ...
      {/* SOS Section - SIMPLIFIED! */}
      <div className="mt-8 mb-4">
        <div className="card p-6 text-center" style={{
          background: "linear-gradient(135deg, #ef4444 0%, #dc2626 100%)",
          border: "none",
        }}>
          <h3 className="text-white font-semibold text-lg mb-2">Emergency SOS</h3>
          
          {!sosLoading && (
            <p className="text-white/90 text-sm mb-4">
              {sosSettings?.sos_enabled ? (
                <>Ready • Contact: {sosSettings.emergency_contact_name || "Set"}</>
              ) : (
          ...
          {/* Simple Edit link that goes to the existing safety page */}
          <Link 
            href="/safety" 
            className="inline-block text-white/90 underline text-sm hover:text-white"
          >
            {sosSettings?.sos_enabled ? "Edit Settings" : "Setup Emergency Contact"}
          </Link>
        </div>
      </div>
